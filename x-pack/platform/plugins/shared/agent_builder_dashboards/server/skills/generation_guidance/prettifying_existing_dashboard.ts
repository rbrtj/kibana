...
3. Compare the ideal design with the existing inventory. Preserve useful panels, proactively add every missing high-value visualization with \`add_panels\`, and remove panels that are clearly redundant or unrelated with \`remove_panels\`. An explicit prettify request authorizes these justified composition changes; do not stop at recommendations or wait for separate approval. Every addition must answer a distinct purpose-relevant question, and uncertain removals should remain recommendations.
